...
if (totals) {
  ...
  if (typeof statements === 'number' && statements < 88) {
    console.error(
      `Coverage thresholds failed: overall statements ${statements}% < 88%`,
    );
    process.exit(1);
  }
  if (typeof lines === 'number' && lines < 90) {
    console.error(`Coverage thresholds failed: overall lines ${lines}% < 90%`);
    process.exit(1);
  }
} else {
